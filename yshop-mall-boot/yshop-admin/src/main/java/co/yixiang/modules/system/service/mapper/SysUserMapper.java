...
public interface SysUserMapper extends CoreMapper<User> {
     ...
     * @param userName 用户名
     */
    @Select("SELECT u.id,u.nick_name,u.sex,u.dept_id,u.enabled,u.create_time,u.phone,u.email,u.job_id ,u.`password` ,u.username,ua.real_name avatar FROM sys_user " +
            " u LEFT JOIN sys_user_avatar ua ON u.avatar_id = ua.id  WHERE u.username = #{username}")
