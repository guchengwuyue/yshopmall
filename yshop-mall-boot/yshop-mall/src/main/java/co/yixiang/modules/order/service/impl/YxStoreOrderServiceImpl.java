...
//        publisher.publishEvent(new TemplateEvent(this,templateBean));
    ...
    public void orderDelivery(String orderId, String deliveryId, String deliveryName, String deliveryType) {
        ...
        // 微信发货信息录入
        String shippingConfig = redisUtils.getY(ShopConstants.YSHOP_WECHAT_ROUTINE_SHIPPING);
        if (StrUtil.isNotBlank(shippingConfig) && ShopConstants.YSHOP_ONE.equals(shippingConfig)){
            LambdaQueryWrapper<YxStoreOrderCartInfo> wrapper = new LambdaQueryWrapper<>();
            wrapper.eq(YxStoreOrderCartInfo::getOid, orderQueryVo.getOrderId());
            List<YxStoreOrderCartInfo> cartInfos = orderCartInfoService
                    .list(wrapper);
            List<YxStoreCartQueryVo> cartInfo = cartInfos.stream()
                    ...
                    .collect(Collectors.toList());
            String productName = cartInfo.stream().map(d -> StrUtil.format("{}*{}件", d.getProductInfo().getStoreName(), d.getCartNum())).collect(Collectors.joining(","));
            if (productName.length() > 120) {
                productName = productName.substring(0, 119);
            }
            TemplateBean shoppingTemplateBean = TemplateBean.builder()
                    .orderId(orderQueryVo.getOrderId())
                    .deliveryId(deliveryId)
                    .deliveryName(expressQueryVo.getName())
                    .phone(orderQueryVo.getUserPhone())
                    .productName(productName)
                    .uid(orderQueryVo.getUid())
                    .templateType(TemplateListenEnum.TYPE_10.getValue())
                    .build();
            publisher.publishEvent(new TemplateEvent(this, shoppingTemplateBean));
        }
    ...
    private BigDecimal getGainIntegral(List<YxStoreCartQueryVo> cartInfo) {
        ...
        for (YxStoreCartQueryVo cart : cartInfo) {
            if (cart.getCombinationId() > 0 || cart.getSeckillId() > 0) {
                continue;
            }
